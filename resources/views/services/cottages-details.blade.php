        ...
        <div class="row">
            <div class="col-md-8">
                <div class="slider">
                    <div id="slider-larg" class="owl-carousel image-gallery">
                        ...
                        <div class="item lightbox-image-icon">
                            <a href="{{asset('assets/images/cottages/'.$data->gallery)}}" class="hover_effect h_lightbox h_blue">
                                <img class="img-responsive" style="height:500px" src="{{asset('assets/images/cottages/'.$data->gallery)}}" alt="Image">
                            </a>
                        </div>
                        @endforeach
                    </div>
                    <div id="thumbs" class="owl-carousel">
                        @foreach($gallery as $data) 
                        <div class="item"><img class="img-responsive" style="height:80px;" src="{{asset('assets/images/cottages/'.$data->gallery)}}" alt="Image"></div>
                        @endforeach
                    </div>
                </div>
                <div class="main_title mt50">
                    <h2 class="text-uppercase">What about this Cottage?</h2>
                </div>
                <p class="text-justify">{{$query->description}}</p>

                <form action="{{url('services/reservation/addons_set_b')}}" method="POST">
                    @csrf
                            ...
                            @foreach($addons_rooms as $data)
                                <div class="col-md-6">
                                    <div class="form-check">
                                        <input class="form-check-input" name="addons_rooms[]" type="checkbox" value="{{$data->id}}" id="rooms{{$data->id}}">
                    ...
                    <button class="button btn_lg btn_blue btn_full" onclick="return confirm('Are you sure want to add this addons?');" type="submit">UPDATE ADD-ONS</button>
                </form>
                ...
                <div class="similar_rooms">
                    <div class="main_title t_style5 l_blue s_title a_left">
                        <div class="c_inner">
                            <h2 class="c_title">SIMILAR COTTAGES</h2>
                        </div>
                    </div>
                    <div class="row">
                            ...
                            <div class="col-md-4">
                                <article>
                                    <figure>
                                        <a href="{{url('cottages')}}/{{Crypt::encryptString($data->id)}}/details" class="hover_effect h_blue h_link"><img src="{{asset('assets/images/cottages/'.$data->image)}}" alt="Image" class="img-responsive"></a>
                                        <figcaption>
                                            <h4 class="text-center"><a href="{{url('cottages')}}/{{Crypt::encryptString($data->id)}}/details">{{$data->title}}</a></h4>
                                        </figcaption>
                                    </figure>
                                </article>
                            </div>
                    ...
                    @if(Session::has('customer_id'))
                        ...
                        <div class="row">
                            <form method="POST" action="{{url('services/comments')}}" data-parsley-validate>
                                ...
                                <input type="hidden" name="category" value="Cottages">
                                <div class="col-md-12">
            ...
            <div class="col-md-4">
                <div class="sidebar">
                    <aside class="widget">
                        <table class="table">
                            <tr>
                                <td class="text-left">Rate / Day (Cottage)</td>
                                <td class="text-right">₱{{number_format($query->rate,2)}}</td>
                            </tr>

                            <tr>
                                <td class="text-left">Add-Ons (Rooms)</td>
                                <td class="text-right">₱<span id="cottage">@if(Session::has('total_rooms_rate')){{number_format(Session::get('total_rooms_rate'),2)}} @else {{number_format(0,2)}} @endif</span></td>
                            </tr>
                            ...
                            <form method="POST" action="{{url('services/reservation')}}" class="inner">
                                ...
                                <div class="form-group col-md-12 col-sm-12 col-xs-12 nopadding">
                                    <div class="form_select">
                                        <div class="form-group">
                                            <input type="hidden" name="adults" value="{{$query->pax}}">
                                            <input class="form-control" name="booking-name" value="{{$query->pax}} Pax" disabled type="text">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-group col-12 nopadding">
                                    <div class="input-group">
										...
										autocomplete=off placeholder="Reservation Date" required>
                                        </div>
                                    </div>
                                </div>
                                ...
                                @if(Session::has('customer_id'))
                                    <button class="button btn_lg btn_blue btn_full" onclick="return confirm('Are you sure want to complete this reservation?');" type="submit">BOOK A COTTAGE NOW</button>
                                @else 
